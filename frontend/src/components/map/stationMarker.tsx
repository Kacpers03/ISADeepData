  ...
  return (
    ...
    >
      <div 
        ...
        ref={markerRef}
        className={`${styles.mapMarker} ${station.contractorAreaBlockId ? styles.associatedMarker : ''} ${isHovered ? styles.markerHover : ''}`}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        <div className={styles.markerIcon}>
          {getStationIcon() && (
            <span className={styles.stationTypeIcon}>{getStationIcon()}</span>
          )}
        </div>
        ...
        <div className={styles.stationTooltip}>
          ...
          <div>Coords: {station.latitude.toFixed(4)}, {station.longitude.toFixed(4)}</div>
        </div>
      </div>
    </Marker>
  );
