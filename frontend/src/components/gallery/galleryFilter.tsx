...
const ImprovedGalleryFilter: React.FC<GalleryFilterProps> = ({
  ...
  const debouncedFilterChange = useCallback(debounce((key: string, value: string) => {
  ...
  return (
    <div className={`${mapStyles.improvedFilterPanel} ${styles.filterContainer} ${isCollapsed ? styles.collapsed : ''}`}>
                  ...
                  <ul>
                    {searchResults.map((result, index) => (
                      <li key={`result-${result.mediaId || index}`} onClick={() => handleResultClick(result)}>
                        ...
                        {result.stationCode && (
                          <div className={mapStyles.resultParent}>Station: {result.stationCode}</div>
                        )}
                      </li>
                    ))}
                  </ul>
