  ...
  useEffect(() => {
    const fetchMediaItems = async () => {
      try {
        ...
        console.log(
          `Attempting to fetch media from: ${API_BASE_URL}/Gallery/media`
        );
        ...
        try {
          ...
          const ALT_API_URL = API_BASE_URL.includes("5062")
            ? "http://localhost:5062/api"
            : "http://localhost:5802/api";

          console.log(
            `Attempting alternative URL: ${ALT_API_URL}/Gallery/media`
          );
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch media. Status: ${response.status} ${response.statusText}`
          );
        }
        ...
        const contractorOptions = Array.from(
          new Set(processedData.map((item: MediaItem) => item.contractorId))
        )
          .filter((id) => id !== null && id !== undefined)
          .map((id) => ({
            id,
            name:
              processedData.find((item: MediaItem) => item.contractorId === id)
                ?.contractorName || "Unknown",
          }));

        const cruiseOptions = Array.from(
          new Set(processedData.map((item: MediaItem) => item.cruiseId))
        )
          .filter((id) => id !== null && id !== undefined)
          .map((id) => ({
            id,
            name:
              processedData.find((item: MediaItem) => item.cruiseId === id)
                ?.cruiseName || "Unknown",
          }));

        const stationOptions = Array.from(
          new Set(processedData.map((item: MediaItem) => item.stationId))
        )
          .filter((id) => id !== null && id !== undefined)
          .map((id) => ({
            id,
            code:
              processedData.find((item: MediaItem) => item.stationId === id)
                ?.stationCode || "Unknown",
          }));
    ...
    const applyFilters = () => {
      ...
      if (filters.contractorId !== "all") {
        filtered = filtered.filter(
          (item) => item.contractorId?.toString() === filters.contractorId
        );
      }
      ...
      if (filters.cruiseId !== "all") {
        filtered = filtered.filter(
          (item) => item.cruiseId?.toString() === filters.cruiseId
        );
      }
      ...
      if (filters.stationId !== "all") {
        filtered = filtered.filter(
          (item) => item.stationId?.toString() === filters.stationId
        );
      }
      ...
      if (filters.searchQuery.trim() !== "") {
        ...
        filtered = filtered.filter(
          (item) =>
            ...
            (item.stationCode &&
              item.stationCode.toLowerCase().includes(query)) ||
            (item.contractorName &&
              item.contractorName.toLowerCase().includes(query)) ||
            (item.cruiseName &&
              item.cruiseName.toLowerCase().includes(query)) ||
    ...
    const fetchMediaItems = async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:5802/api/Gallery/media",
          {
            signal: controller.signal,
          }
        );
        ...
        try {
          ...
          const response = await fetch(
            "http://localhost:5062/api/Gallery/media",
            {
              signal: controller.signal,
            }
          );
                    ...
                    const checkPorts = async () => {
                      ...
                      for (const port of ports) {
                        try {
                          ...
                          console.log(
                            `Testing connectivity to port ${port}...`
                          );
                          const startTime = Date.now();
                          const response = await fetch(
                            `http://localhost:${port}/api/Gallery/media`,
                            {
                              signal: controller.signal,
